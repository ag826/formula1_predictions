...
full_weather_data=pd.read_csv("RAW_DATA/FP2_DATA/FP2_weather_data.csv")  # Changed from FP1 to FP2
...
agg_weather_data.columns = [
    col[0] if col[0] in ['RACEYEAR', 'RACENUMBER'] else 'FP2_' + '_'.join(col).strip('_')  # Changed FP1 to FP2
    for col in agg_weather_data.columns
]

agg_weather_data = agg_weather_data.drop("FP2_SESSIONTYPE_first", axis=1)  # Changed FP1 to FP2
...
full_lap_data=pd.read_csv("RAW_DATA/FP2_DATA/FP2_lap_data.csv")  # Changed from FP1 to FP2
...
final_fp2_data = final_fp2_data.merge(avg_sector_times, on=['RACEYEAR', 'RACENUMBER'], how="left")
...
final_fp2_data = final_fp2_data.merge(pit_summary, on=['RACEYEAR', 'RACENUMBER', 'Driver'], how="left")
...
tyre_stats_pivot.columns = [f"FP2_{stat}_{compound}" for stat, compound in tyre_stats_pivot.columns]
...
track_status_data=pd.read_csv("RAW_DATA/FP2_DATA/FP2_track_status.csv")  # Changed from FP1 to FP2
...
agg_track_status.columns=["RACEYEAR","RACENUMBER","FP2_Red","FP2_SCDeployed","FP2_VSCDeployed","FP2_Yellow"]  # Changed FP1 to FP2
final_fp2_data=final_fp2_data.merge(agg_track_status, on=['RACEYEAR', 'RACENUMBER'], how="left")  # Changed from final_fp1_data to final_fp2_data

final_fp2_data.to_csv("PROCESSED_DATA/final_fp2_data.csv", index=False)  # Changed output filename from FP1 to FP2 