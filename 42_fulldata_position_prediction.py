# NOTES:
# - Predicting on ClassifiedPosition
...
selected_columns = [col for col in df.columns if  col != 'IsWinnerFlag' and col!='Position' and col!='Points'] 
...
X = df.drop(columns=['ClassifiedPosition'])
y = df['ClassifiedPosition']
...
plt.tight_layout()
# Save before showing
plt.savefig('MODEL_EVALUATION/fulldata_feature_importance.png', dpi=300, bbox_inches='tight')
plt.show()
plt.close()

# Save feature importance data
feature_importance.to_csv('MODEL_EVALUATION/fulldata_feature_importance.csv', index=False)
...
with open('BEST_MODEL/fulldata_preprocessor_position_prediction.pkl', 'wb') as f:
    pickle.dump(preprocessor, f)
with open('BEST_MODEL/fulldata_best_model_position_prediction.pkl', 'wb') as f:
    pickle.dump(best_model, f)
with open('BEST_MODEL/fulldata_label_encoder_position_prediction.pkl', 'wb') as f:
...
with open('MODEL_EVALUATION/fulldata_model_metrics.txt', 'w') as f:
    f.write('\n'.join(metrics_text))
