...
full_weather_data=pd.read_csv("FP2_DATA/FP2_weather_data.csv")  # Changed from FP1 to FP2
...
full_lap_data=pd.read_csv("FP2_DATA/FP2_lap_data.csv")  # Changed from FP1 to FP2
...
final_fp2_data=final_fp2_data.merge(pit_summary, left_on=['RACEYEAR', 'RACENUMBER'], right_on=['RACEYEAR', 'RACENUMBER'], how="right" )  # Changed from final_fp1_data to final_fp2_data
...
tyre_stats_pivot.columns = [f"FP2_{stat}_{compound}" for stat, compound in tyre_stats_pivot.columns]  # Changed FP1 to FP2

# Reset index to get back the race and driver columns
tyre_stats_pivot = tyre_stats_pivot.reset_index()

final_fp2_data = final_fp2_data.merge(  # Changed from final_fp1_data to final_fp2_data
    tyre_stats_pivot, 
    left_on=['RACEYEAR', 'RACENUMBER', 'Driver'], 
    right_on=['RACEYEAR', 'RACENUMBER', 'Driver'], 
    how="left"
)
...
track_status_data=pd.read_csv("FP2_DATA/FP2_track_status.csv")  # Changed from FP1 to FP2
...
final_fp2_data.to_csv("final_fp2_data.csv")  # Changed output filename from FP1 to FP2 